...
API_TOKEN = "8906178880:AAHW2KR-6bvweSyeyBNoVLmtliF7TVxwrkI"
...
FIREBASE_CREDENTIALS = {
    ...
    "project_id": "otp-bot-a8c96",
    "private_key_id": "7471913e16eb144e3ab3efd4c83e2987da6ab658",
    "private_key": "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
    "client_email": "firebase-adminsdk-fbsvc@otp-bot-a8c96.iam.gserviceaccount.com",
    "client_id": "111012163821409736092",
    "auth_uri": "https://accounts.google.com/o/oauth2/auth",
    "token_uri": "https://oauth2.googleapis.com/token",
    "auth_provider_x509_cert_url": "https://www.googleapis.com/oauth2/v1/certs",
    "client_x509_cert_url": "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-fbsvc%40otp-bot-a8c96.iam.gserviceaccount.com",
...
def save_data_firebase():
    ...
    try:
        ...
        admin_ref = db_firebase.collection('admin_data').document('categories')
        admin_ref.set(admin_categories)
        
        # Save claimed stats
        stats_ref = db_firebase.collection('admin_data').document('claimed_stats')
        stats_ref.set(claimed_stats)
...
def load_data_firebase():
    ...
    try:
        ...
        if admin_doc.exists:
            admin_categories = admin_doc.to_dict()
        else:
            admin_categories = {}
        ...
        if stats_doc.exists:
            claimed_stats = stats_doc.to_dict()
        else:
            claimed_stats = {}
        ...
        # Initialize empty data if Firebase fails
        db = {}
        admin_categories = {}
        claimed_stats = {}
...
def save_data():
    """Save data to both Firebase and local backup"""
    save_data_firebase()
    save_data_local()
...
def load_data():
    ...
    load_data_firebase()
    # If Firebase loaded empty data, try local
    if not db and not admin_categories and not claimed_stats:
        load_data_local()
...
def send_main_page(chat_id, message_id, user_id):
    ...
    inline_keyboard = [
        ...
        [{"text": "🔄 Refresh Count", "callback_data": "refresh_count"}]
    ]
...
def handle_message(msg):
    ...
    if user_id == ADMIN_ID:
        if text.startswith("/add "):
            ...
            if cat_name in admin_categories:
                ...
                save_data()
                send_message(chat_id, f"🗑️ {cat_name} মুছে ফেলা হয়েছে।")
            else:
                send_message(chat_id, "❌ বাটন পাওয়া যায়নি।")
            return
            ...
            buttons_list = "\n".join([f"• {k}" for k in admin_categories.keys()]) if admin_categories else "কোনো বাটন নেই।"

            analysis_msg = (
                f"📊 <b>বট অ্যানালাইসিস:</b>\n\n"
                f"👥 মোট ইউজার: {total_users}\n"
                f"✅ টাস্ক সম্পন্ন: {completed_users}\n\n"
                f"📋 লাইভ বাটন:\n{buttons_list}\n\n"
                f"📈 Claim Stats:\n{stats_text}"
            )
    ...
    if text.startswith("/start"):
        ...
        inline_keyboard.append([{"text": "✅ Joined", "callback_data": "check_joined"}])

        send_message(
            chat_id,
            "👋 <b>বট ব্যবহার করতে চ্যানেলে জয়েন করুন</b>\n\n"
            "সব চ্যানেলে জয়েন করে <b>Joined</b> বাটনে ক্লিক করুন:",
            {"inline_keyboard": inline_keyboard}
        )
...
def handle_callback(callback):
    ...
    if data == "check_joined":
        if is_user_joined_all(user_id):
                ...
                if ref_id and ref_id in db:
                    ...
                    try:
                        send_message(
                            ref_id,
                            f"🎉 নতুন রেফার সফল!\n"
                            f"বর্তমান রেফার: {db[ref_id]['referral_count']}"
                        )
                    except:
                        pass
            ...
            answer_callback_query(clb_id, "❌ এখনও জয়েন করা হয়নি!", show_alert=True)

    elif data == "refresh_count":
        try:
            send_main_page(chat_id, message_id, user_id)
            answer_callback_query(clb_id, "আপডেট হয়েছে!")
        except:
            answer_callback_query(clb_id, "ইতিমধ্যে আপডেট আছে।")

    elif data == "click_access":
        current_refs = db[user_id]["referral_count"]
        if current_refs >= REQUIRED_REFERRALS:
            if not admin_categories:
                answer_callback_query(clb_id, "⚠️ এডমিন এখনো কিছু সেট করেননি।", show_alert=True)
                return

            text = f"🎉 অভিনন্দন!\n\nআপনার {REQUIRED_REFERRALS} টি রেফার পূর্ণ হয়েছে।"
            inline_keyboard = []
            ...
            answer_callback_query(clb_id, f"❌ এখনও {REQUIRED_REFERRALS} টি রেফার পূর্ণ হয়নি!", show_alert=True)
        ...
        try:
            ...
            if 0 <= cat_index < len(categories_list):
                ...
                answer_callback_query(clb_id, "❌ বাটন খুঁজে পাওয়া যায়নি!", show_alert=True)
        except:
            answer_callback_query(clb_id, "❌ ত্রুটি!", show_alert=True)
    ...
    while True:
        try:
            ...
            if res and res.get("ok"):
                ...
                print("⚠️ getUpdates failed. Sleeping 5 sec...")
                time.sleep(5)
